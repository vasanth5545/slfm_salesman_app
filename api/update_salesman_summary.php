...
// Target Month logic
$target_month = $input_data['target_month'] ?? $_POST['target_month'] ?? $_GET['target_month'] ?? date('Y-m');
$current_date = date('Y-m-d');
$current_month = $conn->real_escape_string($target_month); // YYYY-MM

// Adjust current_day based on the selected month
if ($current_month === date('Y-m')) {
    $current_day = (int) date('d'); // Current month, up to today
} else {
    $current_day = (int) date('t', strtotime($current_month . '-01')); // Past month, up to last day of that month
}
...
$salesman_sql = "SELECT name, showroom_name, created_at FROM salesmen WHERE salesman_id = '$salesman_id'";
...
$created_at = $salesman_row['created_at'] ?? '2000-01-01 00:00:00';
$joining_date = date('Y-m-d', strtotime($created_at));

// 2. Fetch Attendance Records for CURRENT MONTH
$sql = "SELECT date, clock_in_time, clock_out_time, status, modification_reason FROM attendance 
...
$has_punch_on_joining_day = false;
$is_punched_today = false;
...
while ($row = $result->fetch_assoc()) {
    ...
    $mod_reason = $row['modification_reason'] ?? '';

    if ($date == $joining_date)
        $has_punch_on_joining_day = true;
    if ($date == $current_date)
        $is_punched_today = true;

    // Normalize Status Check (Handle 'On Leave', 'Leave', 'Sick Leave' etc as Leave)
    $status_lower = strtolower($status);
    if (strpos(strtoupper($mod_reason), 'M/O SO HALF DAY') !== false) {
        $status_lower = 'half day';
    }

    // --- STRICT CHECK (As per User) ---
    // Previously skipped if out_time is null, but we want to count backdated manual updates.
    /*
    if ($date != $current_date && !empty($in_time) && empty($out_time)) {
...
// ---------------------------------------------------------
// LOGIC: ACCOUNTABILITY START 📅
// ---------------------------------------------------------
$accountability_start = $joining_date;
if (!$has_punch_on_joining_day) {
    // If no work on joining day, accountability starts the NEXT day
    $accountability_start = date('Y-m-d', strtotime('+1 day', strtotime($joining_date)));
}
$calculation_end_date = ($current_month === date('Y-m')) ? $current_date : date('Y-m-t', strtotime($current_month . '-01'));
$month_start = date('Y-m-01', strtotime($current_month . '-01'));
$calc_start_date = ($accountability_start > $month_start) ? $accountability_start : $month_start;

// ---------------------------------------------------------
// LOGIC: HOLIDAY COUNT 📅
// ---------------------------------------------------------
$holidays_count = 0;
// Only count holidays from accountability start up to (maybe) today
if ($current_month === date('Y-m')) {
    $holiday_end_date = $is_punched_today ? $calculation_end_date : date('Y-m-d', strtotime('-1 day', strtotime($calculation_end_date)));
} else {
    $holiday_end_date = $calculation_end_date; // For past month, include all holidays
}

$h_sql = "SELECT count(*) as h_count FROM holidays 
          WHERE holiday_date >= '$calc_start_date' 
          AND holiday_date <= '$holiday_end_date'";
...
// Calculate days passed in current month since accountability start
if ($calc_start_date > $calculation_end_date) {
    $days_passed = 0;
} else {
    $diff = strtotime($calculation_end_date) - strtotime($calc_start_date);
    $days_passed = floor($diff / (60 * 60 * 24)) + 1;
}

// 🔥 CRITICAL: If no punch TODAY, don't count today as a working day in the percentage yet
if ($current_month === date('Y-m') && !$is_punched_today && $days_passed > 0) {
    $days_passed--;
}
...
// 🔥 FIX: Only send sync signal if NOT triggered from attendance.php
// When attendance.php triggers this, Flutter already has the response.
// The RTDB sync signal was causing Flutter to re-fetch and reset image state,
// which is the root cause of photo disappearance.
$skip_sync = $input_data['skip_sync_signal'] ?? false;
if (!$skip_sync) {
    sendSyncSignal($salesman_id);
} else {
    // Skip — Flutter already has the attendance response
}
