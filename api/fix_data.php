...
// ============================================================
// 🔒 PROTECTED SALESMEN — fix_data.php will NEVER modify these
// Owner manually sets their in/out times via manual_fix_attendance.php
// ============================================================
$PROTECTED_SALESMAN_IDS = ['SM234', 'SM454'];
$protected_ids_sql = "'" . implode("','", $PROTECTED_SALESMAN_IDS) . "'"; // For SQL IN clause
...
$att_cols_to_check = [
    ...
    'admin_approval' => "VARCHAR(20) DEFAULT NULL", // Ensuring this exists
    'late_entry_approved' => "TINYINT(1) DEFAULT 0"  // 🔒 Protect admin-approved late entries
];
...
// 6.1 FINAL OUT SELFIE URL (NEW)
$check_final_out_img = $conn->query("SHOW COLUMNS FROM attendance LIKE 'final_out_selfie_url'");
if ($check_final_out_img && $check_final_out_img->num_rows == 0) {
    $sql_final_out_img = "ALTER TABLE attendance ADD COLUMN final_out_selfie_url TEXT DEFAULT NULL";
    if ($conn->query($sql_final_out_img) === TRUE) {
        $response['schema_updates'][] = "Added 'final_out_selfie_url' column";
    }
}
...
if (!empty($input_id)) {
    ...
    // Bulk Mode 🌍 (Exclude protected salesmen)
    $s_sql = "SELECT salesman_id, name, showroom_name FROM salesmen WHERE status = 'Active' AND salesman_id NOT IN ($protected_ids_sql)";
    $response['mode'] = "Bulk Fix (All Active Salesmen, excluding protected: " . implode(', ', $PROTECTED_SALESMAN_IDS) . ")";
}
...
// 9.2 🎯 HOLIDAY AUTO-FIX: Convert 'Absent' → 'Holiday' if date is in holidays table
// User Request: "owner sir than holiday kodutha ru so atha colculate panna theva illa"
$holiday_fix_count = 0;
$holiday_dates_sql = "SELECT holiday_date FROM holidays";
$holiday_dates_res = $conn->query($holiday_dates_sql);
$holiday_dates_array = [];
if ($holiday_dates_res) {
    while ($h_row = $holiday_dates_res->fetch_assoc()) {
        $holiday_dates_array[] = $h_row['holiday_date'];
    }
}

if (!empty($holiday_dates_array)) {
    foreach ($holiday_dates_array as $h_date) {
        // Update ALL attendance records on this date: Absent → Holiday
        $fix_h_sql = "UPDATE attendance SET status = 'Holiday' WHERE date = '$h_date' AND status = 'Absent'";
        $conn->query($fix_h_sql);
        $holiday_fix_count += $conn->affected_rows;
    }
}
$response['holiday_auto_fix'] = "Converted $holiday_fix_count 'Absent' records to 'Holiday' status.";

// 10. TASK 6 FIX
$sql_check = "SELECT id, clock_out_time, status, admin_approval, late_entry_approved FROM attendance WHERE date = '$date' AND clock_out_time IS NOT NULL AND salesman_id NOT IN ($protected_ids_sql)";
$result = $conn->query($sql_check);
$updated_count = 0;
if ($result) {
    while ($row = $result->fetch_assoc()) {
        // 🔥 PROTECT APPROVED RECORDS
        if (!empty($row['admin_approval'])) continue;
        // 🔒 PROTECT ADMIN LATE-ENTRY APPROVED RECORDS
        if ($row['late_entry_approved'] == 1) continue;
...
// 11. TASK 1 FIX
$sql_fix_present = "SELECT id, clock_in_time, admin_approval, late_entry_approved FROM attendance WHERE date = '$date' AND status = 'Present' AND salesman_id NOT IN ($protected_ids_sql)";
...
if ($res_fix) {
    while ($row = $res_fix->fetch_assoc()) {
        ...
        if (!empty($row['clock_in_time'])) {
            ...
            if ($in_time_str > '10:01:00') {
                $correct_status = ($in_time_str > '15:00:59') ? 'Leave' : 'Half Day';
...
$sql_consist = "SELECT a.id, a.clock_in_time, a.clock_out_time, a.status, a.admin_approval, a.late_entry_approved, s.gender 
                FROM attendance a 
                JOIN salesmen s ON a.salesman_id = s.salesman_id 
                WHERE a.clock_in_time IS NOT NULL
                AND a.salesman_id NOT IN ($protected_ids_sql)";
...
if ($res_consist) {
    while ($row = $res_consist->fetch_assoc()) {
        ...
        // 🎯 PROTECT HOLIDAY STATUS (User Request: Don't recalculate holidays)
        if (strtolower($current_status) == 'holiday') continue;
...
foreach ($salesmen_list as $salesman) {
    ...
    while ($row = $att_res->fetch_assoc()) {
        ...
        // 🎯 HOLIDAY EXCLUSION: Ignore holiday records completely
        if ($status_lower == 'holiday') {
            continue; // Don't count holidays as any status
        }
    ...
    // Leave Consumption Logic 🍃
    // --------------------------------------------------------------------------------
    $total_consumed_calc = $leave_count + $absent_count + ($half_day_count * 0.5);
